...
function buildMoodCandles(series, targetCount = 40) {
  ...
  const bucketSize = Math.max(1, Math.floor(series.length / targetCount));
  const candles = [];
  ...
  for (let i = 0; i < series.length; i += bucketSize) {
    ...
    const prices = chunk.map((p) => Number(p.price ?? p)).filter(Number.isFinite);
    if (!prices.length) continue;

    candles.push({
      ts: Number(chunk[0].ts || Date.now()),
      open: prices[0],
...
function getMoodSeriesWithTime(tf) {
  ...
  if (Array.isArray(bucket) && bucket.length >= 2) {
    return bucket
      .filter((b) => Number.isFinite(Number(b.price)) && Number(b.price) > 0)
...
function drawMoodBackdrop() {
  ...
  const series = getMoodSeriesWithTime(moodTokenTimeframe);
  ...
  const candles = useCandles ? buildMoodCandles(series) : [];
  ...
  const x = (i) => MC.left + i * step + step / 2;
  ...
  for (let i = 0; i <= LEVELS; i++) {
    ...
    axisYHtml += `<text x="${(MC.left + plotW + 7).toFixed(1)}" y="${(yy + 3.5).toFixed(1)}">${escapeHtml(formatAxisPrice(value))}</text>`;
  }
  ...
  if (groups.body) {
    if (useCandles && candles.length >= 2) {
      ...
      groups.body.innerHTML = `
        <path d="${path} L ${x(series.length - 1).toFixed(2)} ${baseY} L ${x(0).toFixed(2)} ${baseY} Z"
              fill="${up ? "url(#moodGradUp)" : "url(#moodGradDown)"}"></path>
        <path class="mood-chart-line" d="${path}" stroke="${color}"></path>`;
    }
  }

  // ---------- Volumen ----------
  /* Sale del reparto compra/venta de cada vela. Sin volumen, un
     gráfico de precio cuenta media historia. */
  if (groups.vol && useCandles && candles.length >= 2) {
    const maxTicks = Math.max(...candles.map((c) => c.ticks));
    const barW = Math.max(1, Math.min(step * 0.66, 20));
    const vTop = moodVolTop(), vH = moodVolH();

    groups.vol.innerHTML = candles.map((c, i) => {
      const h = Math.max((c.ticks / maxTicks) * vH, 0.6);
