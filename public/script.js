...
function drawMoodBackdrop() {
  ...
  const sampleLabel = formatAxisPrice(vMax);
  ...
  if (groups.body) {
    if (useCandles && candles.length >= 2) {
      ...
      const gapLimit = Math.max(medianGap * 6, 30000);
      ...
      /* El relleno se traza continuo (un área con huecos no se
         puede cerrar), pero la línea sí los respeta. */
      const areaPath = series
        .map((p, i) => `${i === 0 ? "M" : "L"} ${x(i).toFixed(2)} ${y(p.price).toFixed(2)}`)
        .join(" ");

      groups.body.innerHTML = `
        <path d="${areaPath} L ${x(series.length - 1).toFixed(2)} ${baseY} L ${x(0).toFixed(2)} ${baseY} Z"
              fill="${up ? "url(#moodGradUp)" : "url(#moodGradDown)"}" opacity="0.55"></path>
