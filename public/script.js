...
function renderMoodTradesFeed() {
  ...
  if (!moodTrades.length) {
    ...
    const msg =
      moodTradesSource === "No recent trades"     ? "No trades in the last minutes" :
...
function updateMoodUI() {
  ...
  if (els.volatility) {
    ...
    els.volatility.className =
      ratio >= 2 ? "negative" : ratio >= 1.3 ? "neutral" : "positive";
  }
...
function renderBagSearchResults() {
  ...
  if (!bagSearchResults.length) { box.innerHTML = ""; return; }
...
function renderBagMood() {
  ...
  list.innerHTML = bagMoodHoldings.map((holding, index) => {
    ...
    const volLabel = isVolatileBagCoin(holding) ? "Volatile" : "Major";
      ...
      <div class="bag-row ${active}" data-select-bag="${index}">
        <div class="bag-coin">
          ...
          <div>
            ...
            <span>${escapeHtml(holding.name || "")} · ${volLabel}</span>
          </div>
        </div>
        <div><span class="bag-row-label">Invested</span><strong>${formatCurrency(d.invested)}</strong></div>
        <div><span class="bag-row-label">Value</span><strong>${formatCurrency(d.currentValue)}</strong></div>
        <div><span class="bag-row-label">Entry</span><strong>${formatCurrency(d.entryPrice)}</strong></div>
        <div><span class="bag-row-label">Now</span><strong>${formatCurrency(d.currentPrice)}</strong></div>
        <div class="${cls}"><span class="bag-row-label">PNL</span><strong>${formatPercent(d.pnlPercent)}</strong></div>
        <div class="mood-${coinMood.key}"><span class="bag-row-label">Mood</span><strong>${coinMood.name}</strong></div>
        <button class="bag-remove-btn" type="button" data-remove-bag="${index}">Remove</button>
      </div>`;
  }).join("");
}
...
function drawHistoryChart(series) {
  ...
  /* El eje Y es SIEMPRE 0-100. Autoescalar sería un error grave:
     haría que una franja de 48-52 se viera como una montaña rusa
     y el usuario leería drama donde hay calma. */
  const y = (score) => pad + (1 - clamp(score, 0, 100) / 100) * (h - pad * 2);
  ...
  if (bands) {
    bands.innerHTML = [
      ...
      <text x="8" y="${(y(b.at) - 5).toFixed(1)}">${b.label}</text>
    `).join("");
  }
}
...
function setupMoodTokenControls() {
  ...
  bindOnce(els.copyBtn, "boundCopyCa", "click", async () => {
    try {
      await navigator.clipboard.writeText(MOOD_FIXED_DISPLAY_CA);
...
function startMoodPolling() {
  setTimer("moodMarket", loadMoodMarketSnapshot, 15000);
