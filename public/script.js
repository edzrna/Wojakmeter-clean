...
/* RESTAURADAS.
   ...
   Al reescribir la sección de la bolsa reemplacé un bloque que
   contenía estas dos definiciones y las borré sin darme cuenta.
   El resultado: renderBagMood lanzaba
     ReferenceError: getHoldingValue is not defined
   justo al calcular totalValue.

   Como la excepción ocurría DESPUÉS de pintar la cabecera y
   ANTES de pintar la lista, quedaba el estado imposible:
   cifras arriba y "Build your bag" abajo. */
...
function addBagHolding(coin, usdValue, entryPrice = 0) {
  ...
  /* AVISO DE ENTRADA IMPLAUSIBLE.

     Un +32112% no es un portafolio: es un precio de entrada mal
     tecleado. Antes se aceptaba en silencio y el módulo mostraba
     una euforia falsa que desacredita todo lo demás.

     No se bloquea —puede ser un token que hizo 300x de verdad—
     pero se avisa, porque el caso frecuente es el dedo torcido. */
  const marketPrice = Number(normalized.current_price || 0);
  if (marketPrice > 0) {
    const ratio = marketPrice / entry;
    if (ratio > 100 || ratio < 0.01) {
      const ok = window.confirm(
        `Entry price ${formatCurrency(entry)} vs market ${formatCurrency(marketPrice)} ` +
        `implies ${((ratio - 1) * 100).toFixed(0)}% PNL.\n\nIs that correct?`
      );
      if (!ok) return;
    }
  }
...
function renderBagMood() {
  ...
  /* Este render escribe la cabecera ANTES que la lista. Si algo
     lanza en medio, el usuario ve cifras arriba y vacío abajo
     sin ninguna pista de que hubo un error. El try/catch de
     abajo convierte un fallo silencioso en un mensaje visible. */

  const alloc = byId("bagAllocationBar");

  if (!bagMoodHoldings.length) {
    list.innerHTML = `<div class="bag-empty">Build your bag to see what it feels like.</div>`;

    /* La barra de asignación vacía dejaba una franja gris
       flotando bajo la cabecera, sin explicación. Si no hay
       nada que repartir, no hay barra. */
    if (alloc) {
      alloc.innerHTML = "";
      alloc.parentElement?.classList.add("hidden");
    }

    /* DIAGNÓSTICO: la cabecera mostraba cifras (Euphoria, +32112%)
       mientras la lista decía "Build your bag". Ambas salen del
       mismo array en el mismo render, así que es imposible salvo
       que algo esté mutando el estado entre medias. Este aviso
       lo delata en cuanto vuelva a pasar. */
    if (result.invested > 0 || result.value > 0) {
      console.warn(
        "WM Bag: estado inconsistente — cabecera con datos y lista vacía.",
        { result, holdings: bagMoodHoldings }
      );
    }
    return;
  }

  if (alloc) alloc.parentElement?.classList.remove("hidden");
  ...
  renderBagAllocation(sorted, totalValue);
    ...
    /* Un fallo aquí dejaba la cabecera con cifras y la lista
       vacía, indistinguible de "no tienes nada". Ahora se dice
       lo que pasa en vez de fingir un estado vacío. */
    console.error("WM Bag: fallo al renderizar posiciones", error);
    list.innerHTML = `
      <div class="bag-empty">
        Couldn't render your holdings. Try Reset if this persists.
      </div>`;
  }
}
