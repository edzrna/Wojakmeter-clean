...
function drawCandleChart(prices) {
  ...
  const area = byId("coinChartArea");
  const candles = byId("coinChartCandles");
  if (!candles || !prices || !prices.length) return;
  ...
  candles.innerHTML = sample.map((price, i) => {
    const open  = sample[Math.max(i - 1, 0)];
    ...
    const y = (v) => h - ((v - min) / range) * (h - 24);

    const color = close >= open ? "var(--pos-hard)" : "var(--neg-hard)";
    const rectY = Math.min(y(open), y(close));
    const rectH = Math.max(Math.abs(y(close) - y(open)), 3);

    return `
      <line x1="${x}" y1="${y(high)}" x2="${x}" y2="${y(low)}" stroke="${color}"></line>
...
async function loadCoinDetails() {
  ...
  try {
    ...
    const rawPrices = chartRes?.prices;
