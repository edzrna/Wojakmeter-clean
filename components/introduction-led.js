...
export function mountIntroductionLed(hostElement, stateElement, portraitElement) {
  "use strict";

  const CFG = {
    pitch:       10,      // separacion de celdas en px CSS
    gap:         2,      // hueco entre celdas: hace visible la rejilla
    levels:      4,      // escalones de brillo, como un panel real
    maxAlpha:    0.28,   // brillo del borde; el contenido va enmascarado
    hole:        0,     // margen en px alrededor del heroe que queda oscuro
    holeFade:    100,    // en cuantos px sube el brillo desde el hueco
    edgeFade:    0.22,   // fraccion del borde donde el panel esta a tope
    fps:         24,     // el ojo no pide mas para esto y ahorra bateria
  ...
  /* ---------------------------------------------------------
     ...
     --------------------------------------------------------- */
  function host() { return hostElement; }

  function source() { return portraitElement; }
  ...
  function moodRGB(mood) {
    const hex = getComputedStyle(stateElement).getPropertyValue('--accent').trim();
    if (/^#[0-9a-f]{6}$/i.test(hex)) return [1,3,5].map(i=>parseInt(hex.slice(i,i+2),16));
    return [184,192,203];
  }
  ...
  function mount() {
    ...
    if (!S.ctx) {canvas.remove();S.canvas=null;return false;}
    resize();
    readAxes();
  ...
  const cleanups=[];
  function listen(target,name,fn){target.addEventListener(name,fn);cleanups.push(()=>target.removeEventListener(name,fn));}
  function cleanup(){cancelAnimationFrame(S.raf);cleanups.forEach(fn=>fn());S.canvas?.remove();S.canvas=null;}
  if(disabled() || !mount()) return cleanup;
  const media=window.matchMedia('(prefers-reduced-motion: reduce)');
  S.reduced=media.matches;S.visible=!document.hidden;
  function refresh(){resize();if(S.reduced)drawStill();}
  listen(window,'resize',refresh);
  listen(document,'visibilitychange',()=>{S.visible=!document.hidden;});
  if(typeof ResizeObserver==='function'){const ro=new ResizeObserver(refresh);ro.observe(hostElement);ro.observe(portraitElement);cleanups.push(()=>ro.disconnect());}
  if(typeof IntersectionObserver==='function'){const io=new IntersectionObserver(entries=>{S.onScreen=entries.some(e=>e.isIntersecting);},{rootMargin:'80px'});io.observe(hostElement);cleanups.push(()=>io.disconnect());}
  const mo=new MutationObserver(()=>{if(S.reduced)drawStill();});mo.observe(stateElement,{attributes:true,attributeFilter:['style','data-mood']});cleanups.push(()=>mo.disconnect());
  listen(media,'change',()=>{S.reduced=media.matches;cancelAnimationFrame(S.raf);if(S.reduced)drawStill();else S.raf=requestAnimationFrame(loop);});
  if(S.reduced)drawStill();else S.raf=requestAnimationFrame(loop);
  return cleanup;
}
