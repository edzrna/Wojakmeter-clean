...
export function computeBreadth(coins) {
  const list = (coins || []).filter((c) => Number.isFinite(
...
export function computeIndex(now, history = {}) {
  ...
  /* 3. REGIMEN DE VOLATILIDAD
     ...
     son igual de volatiles. Se le presta el signo del retorno, asi
     que volatilidad alta empeora una caida y exagera un rally. */
  const volZ = robustZ(history.volatility, now.volatility);
  parts.volRegime = volZ === null ? null
    : clamp(volZ / Z_CLAMP, -1, 1) * Math.sign(now.change || 0);
  ...
  parts.volumeAnom = volumeZ === null ? null
    : clamp(volumeZ / Z_CLAMP, -1, 1) * Math.sign(now.change || 0);
