...
export function deriveAxes({ canonicalScore, delta = 0, volatilityZ = 0,
  ...
  /* ===========================================================
     ARREGLADO: "el heroe no reacciona a los timeframes".

     Aqui el divisor era 18 fijo para todas las ventanas. Un
     movimiento de 18 puntos en 24 horas es un dia historico; en 30
     dias es un mes de los normales. Con el mismo divisor, las tres
     pills saturaban: 24H con -17 daba 0,94, 7D con -28 daba 1, y
     30D con -46 daba 1. Los tres al maximo, el personaje temblando
     igual, y la sensacion de que las pills no hacian nada — porque
     efectivamente no hacian nada.

     Ahora cada ventana trae su escala: lo que cuenta como "mucho"
     depende de cuanto tiempo ha tenido para pasar.
     =========================================================== */
  const escala = Number(deltaScale) > 0 ? Number(deltaScale) : 18;
  const move = clamp(Math.abs(delta) / escala, 0, 1);
